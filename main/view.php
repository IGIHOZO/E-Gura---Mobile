...
function and_products_imgs(){
    ...
    if ($sel_prod->rowCount() >=1) {
        ...
        return print_r(json_encode($mmnmn));
    }
}
...
function mob_pro_cat_andr_2($pr_cat){      //======================= ANDROID 2 sub categories products
    ...
    if ($sel_pro_cat->rowCount()>=1) {
        ...
        $vall["and_products_imgs"] = $sub_cats_products;
        print(json_encode($vall));
    }else{
        print(json_encode(""));
    }
}
   ...
   //@ $usr_iid = $_COOKIE['egura_user_id'];
    ...
    if ($sel_cart->rowCount()>=1) {
        ...
        while ($ft_sel_cart = $sel_cart->fetch(PDO::FETCH_ASSOC)) {
                ...
                $cart_client_names[] =  $ft_sel_cart;
        }
        print(json_encode($cart_client_names));
    }else{
        print(json_encode(""));
       }
}

function mob_disp_cart_andr_2($usr_iid){        //================ PAYMENT PARAMETERS ON CART ANDROID 2
    $con = parent::connect();
   //@ $usr_iid = $_COOKIE['egura_user_id'];
    $sel_cart = $con->prepare("SELECT egura_cart.*,egura_products.* FROM egura_cart,egura_products WHERE egura_cart.cart_product=egura_products.product_id AND cart_user='$usr_iid'");
    $ok_sel_cart = $sel_cart->execute();
    if ($sel_cart->rowCount()>=1) {
        ...
        while ($ft_sel_cart = $sel_cart->fetch(PDO::FETCH_ASSOC)) {
                ...
                // $cart_client_names[] =  $ft_sel_cart;
                $mn_cls_ths = new MainUser();
                $cart_client_names =  $mn_cls_ths->user_fname($usr_iid)." ".$mn_cls_ths->user_lname($usr_iid);
                $cart_client_email =  $mn_cls_ths->user_email($usr_iid);
                if ($cnt!=0) {
                    $products = $products.", ".$ft_sel_cart['product_name'];
                }else{
                    $products = $ft_sel_cart['product_name'];
                }
                $ttll+=($ft_sel_cart['product_chipping']+$ft_sel_cart['product_price'])*$ft_sel_cart['cart_quantity'];

                $chip += $ft_sel_cart['product_chipping'];
                $pri += $ft_sel_cart['product_price'];
                $qnnt += $ft_sel_cart['cart_quantity'];

                $cnt++;
        }
        $ttl = ($chip+$pri)*$qnnt;
        $resp_data['client'] = $cart_client_names;
        $resp_data['email'] = $cart_client_email;
        $resp_data['products'] = $products;
        //$resp_data['total_qnty'] = $ttl;
        $resp_data['another_ttl'] = $ttll;
        //$resp_data['client'] = $;
        //print(json_encode($resp_data));
    $MainUser = new MainUser();
   $MainUser->andr_proceed_with_pay_pr_details_andr_2($products,$cart_client_names,$ttll,$cart_client_email);
...
if (isset($_GET['andr_final_cart'])) {     //===================== RETURN FINAL CART VALUES ANDROID 2
    $main_andr = new MainSystemData();
    $main_andr-> mob_disp_cart_andr_2($_GET['user_id']);
}
...
function andr_proceed_with_pay_pr_details_andr_2($pnm,$upnm,$amnt,$eml){
...
<PTL>15</PTL>
<PTLtype>hours</PTLtype>
<customerFirstName>$upnm</customerFirstName>
<customerLastName></customerLastName>
<customerZip>250</customerZip>
<customerCity>Kigali</customerCity>
<customerCountry>RW</customerCountry>
<customerEmail></customerEmail>
</Transaction>
<Services>
...
if (isset($_GET['andr_proceed_with_pay_pr_details'])) {         //============ Payment for android 
    $MainUser = new MainUser();
    $andr_proceed_with_pay_pr_details = $MainUser->andr_proceed_with_pay_pr_details($_GET['product_id'],$_GET['user_id'],$_GET['quantity']);
}


if (isset($_GET['cart_pay_andr_2'])) {  //============ CART Payment  for android 2
    $MainUser = new MainUser();
    $andr_proceed_with_pay_pr_details = $MainUser->andr_proceed_with_pay_pr_details_andr_2($_GET['product_id'],$_GET['user_id'],$_GET['quantity'],$_GET['quantity']);
}
